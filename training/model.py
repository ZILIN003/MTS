...
class ResNet(nn.Module):  # Follow CutMix
    ...
    def __init__(self, model, num_classes, dataset):
        super(ResNet, self).__init__()
        num_classes = num_classes
        block, num_blocks = self.model_dict[model][0], self.model_dict[model][1]
        self.residual_len = len(num_blocks)
        self.dilation = 1
        norm_layer = nn.BatchNorm2d

        if 'resnext' in model:
            self.base_width = self.model_dict[model][2]  # width per group
            self.groups = self.model_dict[model][3]
        else:
            self.base_width = 64
            self.groups = 1
        ...
        if self.residual_len == 4:
            ...
            self.layer2 = self._make_layer(
                block, 64 * 2, num_blocks[1], stride=stride_list[1], dilate=replace_stride_with_dilation[0],
                norm_layer=norm_layer)
            self.layer3 = self._make_layer(
                block, 64 * 4, num_blocks[2], stride=stride_list[2], dilate=replace_stride_with_dilation[1],
                norm_layer=norm_layer)
            self.layer4 = self._make_layer(
                block, 64 * 8, num_blocks[3], stride=stride_list[3], dilate=replace_stride_with_dilation[2],
                norm_layer=norm_layer)
            self.fc = nn.Linear(
...
class MLP(nn.Module):
    def __init__(self, hidden_size=100, num_layers=1, input_dim=1, num_classes=None, args=None):
        super(MLP, self).__init__()
        self.first_hidden_layer = HiddenLayer(input_dim, hidden_size, args.meta_dropout)
        self.rest_hidden_layers = nn.Sequential(*[HiddenLayer(hidden_size, hidden_size, args.meta_dropout) for _ in range(num_layers - 1)])
        self.output_layer = nn.Linear(hidden_size, 1)
        self.args = args

        self.class_embed = nn.Embedding(num_classes, args.class_embed_size)
        self.group_embed = nn.Embedding(2, 1)
